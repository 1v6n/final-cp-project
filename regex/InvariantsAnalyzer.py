...
def parse_args():
    """
    Lee los arguments de línea de commandos.
    ...
    parser.add_argument("--log", required=True, help="Ruta al archivo de log")

    parser.add_argument(
        "--inv", required=True, help="Ruta al archivo YAML de invariantes"
    )
...
def count_invariants(sequence):
    ...
    while True:
        ...
        if took_upper and took_left:
            ...
            raise RuntimeError(
                "Match encontrado pero no se pudo clasificar la rama del invariante"
            )
...
def main():
    ...
    report_results(
        count=count, counters=counters, remainder=remainder, expected=EXPECTED_RUNS
    )
